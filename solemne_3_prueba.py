...
# Página de Visualización Gráfica
    ...
    x_slide = st.slider(f"Limite inferior del eje X", min_value=min_x, max_value=max_x, value=(min_x, max_x))
    y_slide = st.slider(f"Limite inferior del eje Y", min_value=min_y, max_value=max_y, value=(min_y, max_y))


    # Filtrar los datos según los rangos seleccionados
    df_filtrado = df_paises[ (df_paises[x_var] x_slide) & (df_paises[y_var] y_slide)
    ]
