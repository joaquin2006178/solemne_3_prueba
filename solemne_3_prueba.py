...
st.title("Proyecto de Visualización y Análisis de Datos de Países")
...
if pagina_seleccionada == "Descripción":
    ...
    st.write("""
    Este proyecto utiliza datos sobre países obtenidos de una API pública, que incluyen información sobre la población, área, fronteras, idiomas y zonas horarias.
    La aplicación permite explorar los datos a través de análisis estadísticos, filtrado y gráficos interactivos.
    """)
...
# Página de Interacción con Datos
    ...
    valor_minimo = st.slider(f"Valor mínimo para {columna_filtrar}", min_value=int(df_paises[columna_filtrar].min()), max_value=int(df_paises[columna_filtrar].max()), value=int(df_paises[columna_filtrar].min()))
...
# Página de Visualización Gráfica
    ...
    if tipo_grafico == "Barra":
        ...
        ax.plot(df_filtrado[x_var], df_filtrado[y_var])
        ax.set_xlabel(x_var)
        ax.set_ylabel(y_var)
        ax.set_xlim(x_min, x_max)
        ax.set_ylim(y_min, y_max)
        ...
        ax.pie(df_filtrado[y_var], labels=df_filtrado[x_var], autopct='%1.1f%%', startangle=90)
        ax.axis('equal')
