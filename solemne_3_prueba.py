...
# Página de Visualización Gráfica
    ...
    x_min = st.slider(f"Limite inferior del eje X", min_value=min_x, max_value=max_x, value=(min_x, max_x))
    y_min = st.slider(f"Limite inferior del eje Y", min_value=min_y, max_value=max_y, value=(min_y, max_y))


    # Filtrar los datos según los rangos seleccionados
    df_filtrado = df_paises[ (df_paises[x_var] >= min_x) & (df_paises[x_var] <= max_x) &
                              (df_paises[y_var] >= min_y) & (df_paises[y_var] <= max_y)
    ]
